...
class ScheduledPostTests(unittest.TestCase):
    ...
    def test_due_selection_uses_earliest_scheduled_post(self):
        schedule = {
            "posts": [
                {"publish_date": "2026-08-06", "slug": "later", "status": "scheduled"},
                {"publish_date": "2026-08-01", "slug": "first", "status": "scheduled"},
                {"publish_date": "2026-07-30", "slug": "done", "status": "published"},
            ]
        }
        first = select_due_post(schedule, "2026-08-01")
        self.assertIsNotNone(first)
        self.assertEqual(first["publish_date"], "2026-08-01")
        self.assertIsNone(select_due_post(schedule, "2026-07-31"))
